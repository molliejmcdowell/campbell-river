# R script to load data for ET computation excercise
...
# Suggestion for evaluating data in R:
# the package "plyr" provides some nice functionality for deriving information from groups of data. 
# Below is an example of how to determine daily mean air temperature for this dataset 
# **- note that you must have the package "plyr" installed for this to work
library(plyr)
#compute summary stats for parameter "Ta.degC" from the data.frame "CRdata" for each unique "jday", and store in a new data.frame
# - the function ddply will take information contained in one data.frame and derive a new data.frame based on stats requested
?ddply
# *** see also http://www.cookbook-r.com/Manipulating_data/Summarizing_data/
Ta.daily <- ddply(CRdata, c("jday"), summarise,
                N    = length(Ta.degC),
                mean = mean(Ta.degC),
                sd   = sd(Ta.degC),
                se   = sd / sqrt(N) )

towerET <- ddply(CRdata, c("jday"), summarise,
...
#  3. Calculate daily PET using EcoHydRology [R package].
#  Note: Install package and use the function "PET_fromTemp"
...
#  (i) streamflow
#  (ii) groundwater
#  (iii) eddy correlation (tower)
# and (iv) one estimate of PET
#
# You will want to compare these estimates as we have compared other types of measurements. You can compute means
# and SDs for each estimator, and run statistics
