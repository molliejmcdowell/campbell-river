...
# Author Iain Hawthorne (30/09/2014)
...
# Step 3: Using the cursor interactively select (press once) the desired start point for each slope, press "finish" on plot when complete
#         
...
CO2.all <- read.csv("http://ecohydro.ires.ubc.ca/pdfs/2015,ENVR420,CO2data.csv")
...
CO2.exp$Reading.number <- c(1:11) # assign the "Reading number" to the data.frame
CO2.exp$Treatment <- c(rep(c("grass","bare"),3),rep("meadow",3),rep("raised.bed",2)) #populate the variable "Treatment"
...
# load ancillary data (soil moisture and soil temperature)
CO2.ancil <- read.csv("http://ecohydro.ires.ubc.ca/pdfs/soil.CO2.ancillary.2015.csv")

#suggestion: aggregate CO2.ancil by Reading.number to determine means for VWC and soil temp, 
# then merge these together with the CO2 data to facilitate further analysis. E.g. was VWC related to soil respiration, etc.?
# But how? read below
#Install a package called plyr (lower left window of RStudio, click "packages", then click "install", then type "plyr" and click "Install")
library(plyr)
#now will compute means of the three soil water and soil temp measurments corresponding to each soil CO2 flux
soil.ancil.means <- ddply(CO2.ancil, "Reading.number", summarize, mean = mean(VWC.pct))
names(soil.ancil.means) <- c("Reading.number","VWC.pct")
soil.ancil.means$soil.temp <- as.numeric(daply(CO2.ancil, "Reading.number", summarize, mean = mean(Temp.degC)))

#Let's now combine into one data.frame
CO2.exp <- merge(soil.ancil.means,CO2.exp)
...
# What is the relative influence of soil cover compared to soil temperature and soil water as controlling factors on soil flux?
# This can be explored by breaking it up into parts:
# is there any relationship between soil temperature and soil flux?
plot(CO2.exp$soil.temp,CO2.exp$soil.flux)
# is there any relationship between soil water and soil flux?
plot(CO2.exp$VWC.pct,CO2.exp$soil.flux)
